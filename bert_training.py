...
def get_roberta_output(roberta_model, tokenizer, nlu_t, hds, max_seq_length):
    """
    Here, input is toknized further by WordPiece (WP) tokenizer and fed into BERT.
    INPUT
    :param model_bert:
...
def generate_inputs_roberta(tokenizer, nlu1_tok, hds1):
    tokens = []
    segment_ids = []

    # tokens.append("[CLS]")
    i_st_nlu = len(tokens) + 1  # to use it later

    tokens = tokenizer.encode(nlu1_tok, add_prefix_space=True, is_pretokenized=True)
